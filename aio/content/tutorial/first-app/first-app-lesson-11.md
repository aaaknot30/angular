...
1.  In `src/app/housing-location/housing-location.component.ts`, add an anchor tag to the `section` element and include the `routerLink` directive:
    ...
    For the `routerLink` to work in the template, add a file level import of `RouterLink` and `RouterOutlet` from '@angular/router', then update the component `imports` array to include both `RouterLink` and `RouterOutlet`.  Be sure to add the RouterModule to the housing-location.component.ts file to enable the routerLink.  This involves importing it at the top of the file and adding it to the imports array in the middle of the file.
